...
run:
	bash packagesize.sh
install: build
	sudo gdebi --non-interactive packagesize_UNSTABLE.deb
uninstall:
	sudo apt-get purge packagesize
installed-size:
...
build-deb:
	...
	cp -vf packagesize.sh ./debian/usr/bin/packagesize
	# make the program executable
	chmod +x ./debian/usr/bin/packagesize
	...
	cp -v debian.deb packagesize_UNSTABLE.deb
	rm -v debian.deb
	rm -rv debian
